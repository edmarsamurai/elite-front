...
export function Focus() {
    ...
    async function loadFocusTimes(currentDay: string) {
    ...
    const metricsInfoByDay = useMemo(() => {
        ...
        if (timesMetrics.length) {
            for (const { timeFrom, timeTo } of timesMetrics) {
                const diff = timeFrom.diff(timeTo, 'minutes')
    ...
    useEffect(() => {
        loadFocusMetrics(currentDate.toISOString())
    }, [currentDate])

    useEffect(() => {
        loadFocusTimes(currentDay.toISOString())
    }, [currentDay])
        ...
        <div className={styles.container}>
            <div className={styles.content}>
                ...
                <div className={styles['button-group']}>
                    ...
                    {
                        timerState === TimerState.FOCUS && <Button onClick={handleRest}>Iniciar Descanso</Button>
                    }
                    {
            ...
            <div className={styles.metrics}>
                ...
                <div className={styles['info-container']}>
                    ...
                    <Info value={`${metricsInfoByDay.totalTimeInMinutes} minutes`} label="Tempo total de foco" />
                </div>
                <div className={styles['calendar-container']}>
                    <Calendar
                        getDayProps={(date) => ({
                            selected: dayjs(date).isSame(currentDay),
                        ...
                        renderDay={(date) => {
                            ...
                            return (
                                <Indicator size={10} color="var(--info)" offset={-2}
                                    disabled={!isSameDate}>
                                    <div>{day}</div>
                                </Indicator>
                            );
